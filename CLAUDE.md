...
### 毎朝パトロール（Morning Routineと同時実行）

`おはよう` で起動した際、Morning Routineと並行して以下をチェックし結果を報告する。

1. **重複・役割衝突チェック**: `.claude/agents/` と `.claude/commands/` のファイル名・descriptionを走査し、同一役割が2つ以上あれば指摘
2. **フロントマター欠落チェック**: `.claude/agents/` 配下でYAMLフロントマター（`---`）がないファイルを検出（Anthropic公式仕様違反）
3. **ゴミファイルチェック**: `.DS_Store` / 古いhandoff（3日以上前） / 未使用の一時ファイルを検出
4. **コンテキスト肥大チェック**: `CLAUDE.md` が200行超 / agents 20体超 / commands 50個超になったら警告
5. **クライアントファイル鮮度チェック**: `last_updated` が14日以上前のクライアントファイルを指摘
...
| スキル | 起動ワード | 担当 | 詳細 |
|---|---|---|---|
| **記事CKO君** v3.0 | 「記事FB統合判定して」 | 全体統合（リーダー） | 診断→治療→テスト設計 一気通貫。GO/REVISE/BLOCK判定 |
...
| **記事ライター君** v1.0 | （CKO自動起動） | must_fix #1治療 | HTML書き換え × N1コピー × 1テスト=1変数 |
| **記事バリデーター君** v1.0 | （CKO自動起動） | 書き換え検証 | 品質5項目 + 薬機法 → PASS/REVISE/BLOCK |
| **記事テスト君** v1.0 | （CKO自動起動） | ABテスト設計 | テストカード + Squad Beyond手順書 |

**3フェーズ設計**
- Phase 1（診断）: 8エージェント並列 → CKO統合判定（~60-90秒）
- Phase 2（治療）: ライター君 → バリデーター君 → CKOレビュー（REVISE時のみ）
- Phase 3（テスト設計）: テスト君 → Squad Beyond手順書（治療PASS時のみ）
