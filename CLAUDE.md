...
5. `おはよう` の場合 → `/morning` + `/morning-stocks` + 毎朝パトロール（後述）を並行実行
...
4. CLAUDE.md 200行超 / agents 20体超 / commands 50個超 / rules 6ファイル超 → 警告
5. clients/ の `last_updated` 14日超 → 指摘
6. `./scripts/check_client_data.sh` 実行 → 未登録データソース検出 → 登録候補を提示
...
登録済み: sawada-co / camicks / ameru / gungun / onmyskin / proust / mauri / yomite / sakura
