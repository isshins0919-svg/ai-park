...
```
Amazon部長クン v3.0（総指揮官）
│
├── 【Phase 0: Amazon理解 × 現状監査】
│   └── Amazon算命師   — A9アルゴリズム × CTR/CVR × 現状ギャップ分析
│
├── 【Phase 1: 商品ページ完成】
│   ├── KW層分析君     — 3層KW × コピー強度 × 層別重みスコア
│   ├── 薬機法チェック君— 全コピー自動走査 × NG表現検出 × OK変換提示
│   ├── コピー君       — タイトル / 箇条書き5本 / 商品説明文（層別最適化）
│   ├── 画像評価君     — 7軸29点スコア × ブランド美学 × サムネイル視認性
│   ├── 画像生成君     — Gemini pro-image × 差別化空白地帯 × v3.0プロンプト
│   └── ジャッジ君     — 4ブロッカー × 3条件GO × 3ループ上限
│
├── 【Phase 2: 流入設計（出品後）】
│   ├── SEO君          — バックエンドKW250字 / タイトルKW配置 / 順位目標
│   └── 広告君         — オートキャンペーン → マニュアル移行設計
│
...
ad = {
    "phase1_auto": {
        ...
        "daily_budget": 1000,
        "goal": "勝ちKW特定（データ収集）",
        "kpi_target": "ACOS 50%以内"
    },
    "phase2_manual": {
        "period": "1ヶ月後〜",
        "daily_budget": 2000,
        "target_kw": "オートの購入KW TOP10",
        "kpi_target": "ACOS 30%以内 / ROAS 300%以上"
    },
    "coupon": "10%OFFクーポン常時 → CTR約15%UP"
}
```
...
- Amazon攻略PDF（`brew install poppler` 後に `.claude/knowledge/amazon-algorithm.md` に追記）
