...
### Step 2.5: kiji-rag 参照の取得（選定したhookVectorごと）

選定した各 hookVector について、**kiji-rag（記事LP Vector RAG）から「似た訴求軸で書かれた過去記事チャンク」を引き、FV/教育セクション設計の参考にする**。

```bash
# 各 hookVector について実行
python3 .claude/knowledge/kiji-rag/tools/search.py "<hookVector.text>" \
  --layer A --per-article-limit 2 -k 5 --json
```

**引き方の指針**:
- `layer=A`（共感層）で **記事冒頭の悩み共感フック** を引く
- `per-article-limit 2` で同一記事ばかりにならないよう記事多様性を担保
- 追加で `layer=C` で独自メカニズム、`layer=E element=E3_risk_reversal:0.7` でクロージング形など、FV後のセクションごとに検索を変える

**活用方法**:
- 結果は `lpSpec.kijiRagReferences[]` に保存
- 各 LP の FV 設計・教育セクション設計時に「この訴求軸、他記事でどう書かれてるか」を実例付きで参照
- コピー生成時のプロンプトコンテキストに引用して精度向上

**スキップ条件**:
- kiji-rag が空（記事未登録）の場合はスキップ、エラーにしない
- 対象ジャンルの記事が0件でもエラーにせず、通常通り次ステップへ
