...
```
Amazonキャプテン v3.1（Amazonブレイン × 総指揮官）
│
...
│   ├── 薬機法チェック（全コピー走査）
│   ├── KW層別スコア算出
│   └── ジャッジクルー: 4ブロッカー + GO3条件
│
├── 【Phase 5: 流入設計（出品後）】
│   ├── SEOクルー          — バックエンドKW250字 / タイトルKW配置 / 順位目標
│   ├── 広告クルー         — 6キャンペーン構成 / ROAS管理
│   ├── セール戦略クルー   — クーポン禁止 / 価格探索 / 年間イベント
│   └── CTR改善クルー      — メイン画像最適化 / A/Bテスト
│
├── 【Phase 6: 転換率 × レビュー（出品後30日）】
│   ├── レビュークルー     — Vine申請 / フォローアップ設計 / Q&A初期対応
│   ├── A+コンテンツクルー — ブランドストーリー × 比較表 × 素材科学
│   └── SEO加速クルー      — 外部流入 / ファンくる / 順位モニタリング
│
└── 【Phase 7: PDCA（週次継続）】
    └── データクルー       — KPI読み(CTR/CVR/ROAS/Review) → must_fix #1 指示
```
...
```
B1: 薬機法チェッククルー = BLOCK → 即BLOCK（絶対通過不可）
...
コピークルーの出力は **必ず薬機法チェッククルーを通過** してから部長に返す。
...
ad = {
    ...
    "reporting": {
        ...
        "ダ���ンロード": "SP広告 > 検索��語レポート > ROAS降順���分析"
    }
}
```
...
```python
sale_strategy = {
    "role": "セールスケジュール設計 × クーポン禁止チ��ック × 価格探索設計",
...
agent_routing = {
    "cvr": "画像評価クルー + ジャッジクルー → サブ画像再評価・再生成",
    "sessions_daily": "SEOクルー + 広告クルー → KW追加・予算UP",
    "organic_rank": "SEOクルー → タイトル・バックエンドKW最適化",
    "review_count": "レビュークルー → Vine確認・フォローアップ再設計",
    "review_avg": "A+コンテンツクルー → Q&Aレビュー分析・商品改善提案",
    "roas": "広告クルー → 勝ちKWへ集中・負けKWOFF"
}
```
