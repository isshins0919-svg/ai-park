...
- 9エージェントCKOシステム（Gate 1-5 ファネル並列 + CKO逐次）でスコアカード出力
  Gate 1 STOP: フッククルー（Haiku）
  Gate 2 FLOW: フロークルー（Haiku）— 認知負荷ゼロ評価
...
def run_agent_hook(text: str, client: anthropic.Anthropic, cko_directive: dict = {}) -> dict:
    ...
    return parse_json_safe(raw, {"agent": "記事フッククルー", "hook_score": 50, "error": "parse_failed"})
...
def run_agent_narrative(text: str, client: anthropic.Anthropic, cko_directive: dict = {}) -> dict:
    ...
    return parse_json_safe(raw, {"agent": "記事アーククルー", "narrative_score": 50, "error": "parse_failed"})
...
def run_agent_trust(text: str, client: anthropic.Anthropic, cko_directive: dict = {}) -> dict:
    ...
    return parse_json_safe(raw, {"agent": "記事トラストクルー", "trust_score": 50, "error": "parse_failed"})
...
def run_agent_cta(text: str, client: anthropic.Anthropic, cko_directive: dict = {}) -> dict:
    ...
    return parse_json_safe(raw, {"agent": "記事CTAクルー", "cta_score": 50, "error": "parse_failed"})
...
def run_agent_offer(text: str, client: anthropic.Anthropic, cko_directive: dict = {}) -> dict:
    ...
    return parse_json_safe(raw, {"agent": "記事オファークルー", "offer_score": 50, "error": "parse_failed"})


# ── Agent 06: 記事コンパスクルー（API不使用・JSONルックアップのみ）──────
...
FLOW_SYSTEM = """あなたは記事フロークルー — 記事LPの認知負荷を評価する専門エージェントだ。
    ...
    for attempt in range(3):
        try:
            ...
            if raw.startswith("```"):
                ...
                if raw.startswith("json"):
                    raw = raw[4:]
            return parse_json_safe(raw.strip(), {"agent": "記事フロークルー", "flow_score": 50, "error": "parse_failed"})
        except anthropic.RateLimitError:
            if attempt < 2:
                time.sleep(30 * (attempt + 1))
            else:
                return {"agent": "記事フロークルー", "flow_score": 50, "error": "rate_limited"}
        except Exception as e:
            log.warning(f"フロークルーエラー: {e}")
            return {"agent": "記事フロークルー", "flow_score": 50, "error": str(e)}
    return {"agent": "記事フロークルー", "flow_score": 50, "error": "retry_exceeded"}


# ── Agent 00 (CKO前処理): 記事CKOクルー — 指示書生成 ──────────────────────────────
...
JSONのみで返してください:
{
  ...
  "agent_directives": {
    "hook": "フッククルーへ: 特に見てほしいポイントを1文で",
    "flow": "フロークルーへ: 認知負荷の観点で特に見てほしいポイントを1文で",
    "narrative": "アーククルーへ: 特に見てほしいポイントを1文で",
...
Gate評価（読者体験ファネル順）— must_fixは最も早いGateの問題を優先:
  Gate 1: STOP（フッククルー）→ 止まらなければ先は読まれない
  Gate 2: FLOW（フロークルー）→ 脳死で入らなければ訴求が届かない
  Gate 3: FEEL（アーククルー）→ 感情が動かなければ買いたくならない
  Gate 4: TRUST（トラストクルー）→ 信じなければ行動しない
  Gate 5: ACT（CTAクルー+オファークルー）→ 背中を押されなければ買わない
...
# ── Agent 10 (Phase 2): 記事ライタークルー — must_fix #1 HTML書き換え ──────────────
REWRITER_SYSTEM = """あなたは記事ライタークルー — must_fix #1の治療専門エージェントだ。
...
def run_agent_rewriter(must_fix: dict, n1_profile: dict, original_html: str, client: anthropic.Anthropic) -> dict:
    """記事ライタークルー: must_fix #1のHTMLをN1に刺さるコピーに書き換え"""
    user = (
    ...
    for attempt in range(3):
        try:
            ...
            if raw.startswith("```"):
                ...
                if raw.startswith("json"):
                    raw = raw[4:]
            return parse_json_safe(raw.strip(), {"agent": "記事ライタークルー", "error": "parse_failed"})
        except anthropic.RateLimitError:
            if attempt < 2:
                time.sleep(60 * (attempt + 1))
            else:
                raise
    return {"agent": "記事ライタークルー", "error": "retry_exceeded"}


# ── Agent 11 (Phase 2): 記事バリデータークルー — 書き換え検証 × 薬機法 ──────────────
VALIDATOR_SYSTEM = """あなたは記事バリデータークルー — 書き換え結果の品質管理の最後の砦だ。
...
def run_agent_validator(rewriter_output: dict, n1_profile: dict, must_fix: dict, client: anthropic.Anthropic) -> dict:
    """記事バリデータークルー: ライタークルーの書き換え結果を品質+薬機法で検証"""
    user = (
    ...
    for attempt in range(3):
        try:
            ...
            if raw.startswith("```"):
                ...
                if raw.startswith("json"):
                    raw = raw[4:]
            return parse_json_safe(raw.strip(), {"agent": "記事バリデータークルー", "verdict": "REVISE", "error": "parse_failed"})
        except anthropic.RateLimitError:
            if attempt < 2:
                time.sleep(60 * (attempt + 1))
            else:
                raise
    return {"agent": "記事バリデータークルー", "verdict": "REVISE", "error": "retry_exceeded"}


# ── Agent 12 (Phase 3): 記事テストクルー — ABテスト設計 ──────────────────────────
TESTER_SYSTEM = """あなたは記事テストクルー — ABテスト設計の専門家だ。
...
def run_agent_tester(rewriter_output: dict, must_fix: dict, n1_profile: dict, article_url: str, client: anthropic.Anthropic) -> dict:
    """記事テストクルー: PASS済み書き換えのABテスト設計 + Squad Beyond手順書"""
    user = (
    ...
    for attempt in range(3):
        try:
            ...
            if raw.startswith("```"):
                ...
                if raw.startswith("json"):
                    raw = raw[4:]
            return parse_json_safe(raw.strip(), {"agent": "記事テストクルー", "error": "parse_failed"})
        except anthropic.RateLimitError:
            if attempt < 2:
                time.sleep(60 * (attempt + 1))
            else:
                raise
    return {"agent": "記事テストクルー", "error": "retry_exceeded"}
...
def format_cko_output(agents: dict, cko: dict) -> str:
    ...
    lines += ["### 📊 スコアカード",
        ...
        "| エージェント | スコア | 基準 | 判定 |",
        "|---|---|---|---|",
        f"| 記事フッククルー | {s('hook_score', hook, 'hook_score')}点 | ≥75 | {mark(s('hook_score', hook, 'hook_score'), 75)} |",
        f"| 記事フロークルー | {s('flow_score', flow, 'flow_score')}点 | ≥70 | {mark(s('flow_score', flow, 'flow_score'), 70)} |",
        f"| 記事アーククルー | {s('narrative_score', narrative, 'narrative_score')}点 | ≥70 | {mark(s('narrative_score', narrative, 'narrative_score'), 70)} |",
        f"| 記事トラストクルー | {s('trust_score', trust, 'trust_score')}点 | ≥70 | {mark(s('trust_score', trust, 'trust_score'), 70)} |",
    ...
    # フロークルー詳細（Gate 2: FLOW）
    if flow and not flow.get("error"):
        lines += [f"## 🌊 記事フロークルー — {flow.get('flow_score', '—')}点", ""]
    ...
    # フッククルー詳細
    lines += [f"## 📌 記事フッククルー — {hook.get('hook_score', '—')}点", ""]
    ...
    # アーククルー詳細
    lines += [f"## 📈 記事アーククルー — {narrative.get('narrative_score', '—')}点", ""]
    ...
    # トラストクルー詳細
    lines += [f"## 🔒 記事トラストクルー — {trust.get('trust_score', '—')}点", ""]
    ...
    # CTAクルー詳細
    lines += [f"## 🎯 記事CTAクルー — {cta.get('cta_score', '—')}点", ""]
    ...
    # オファークルー詳細
    lines += [f"## 💰 記事オファークルー — {offer.get('offer_score', '—')}点", ""]
    ...
    # コンパスクルー詳細
    lines += [f"## 🏆 記事コンパスクルー — {competitive.get('competitive_score', '—')}点", ""]
...
def run_cko_pipeline(article_data: dict, genre_info: dict, competitor: dict | None, client: anthropic.Anthropic, video_thumbnails: list = [], article_url: str = "") -> str:
    ...
    if verdict == "REVISE":
        ...
        if must_fix_list:
            log.info("Phase 2: 治療開始 — must_fix #1をライタークルーに渡す")
            ...
            # Step 3: ライタークルー（Sonnet）— HTML書き換え
            log.info(f"Phase 2: ライタークルー起動 — セクション: {must_fix_info['section']}")
            rewriter_output = run_agent_rewriter(must_fix_info, n1, target_html, client)

            if not rewriter_output.get("error"):
                # Step 4: バリデータークルー（Haiku）— 品質+薬機法検証（最大2回ループ）
                for validate_loop in range(2):
                    log.info(f"Phase 2: バリデータークルー起動（ループ{validate_loop + 1}回目）")
                    ...
                    if validator_verdict == "PASS":
                        ...
                        # REVISE — ライタークルーに差し戻し
                        if validate_loop < 1:
                            log.info(f"Phase 2: REVISE — ライタークルーに差し戻し（理由: {validator_output.get('revise_reason', '?')}）")
                            # 差し戻し理由を含めて再実行
                ...
                log.warning(f"Phase 2: ライタークルーエラー — {rewriter_output.get('error')}")
    ...
    if treatment_result.get("status") == "PASS":
        log.info("Phase 3: テスト設計開始 — テストクルー起動")
