...
## 📖 用語ルール（2026-04-20 一進さんFB反映・全チャプター適用）

初心者FBで詰まった用語: npm / pip / パッケージ / settings.json / 環境構築 / Node.js / Python / CLI / コマンド・引数 / サーバー

**ルール**:
1. **初出時に本文内で補足**: `用語（わかりやすい補足）` の形式
   - 例: `CLI（シーエルアイ / Command Line Interface = ターミナルに文字を打ってPCを操作する画面）`
   - 例: `npm（JavaScript用のアプリ配布所。スマホのApp Storeみたいなもの）`
2. **用語集(glossary)に必ず登録**: 本文で補足した用語はすべて章末の glossary にも記載
3. **例え話で説明**: 専門用語 → 日常の例え（App Store、スマホ、家の基礎工事、裏方スタッフなど）
4. **初心者目線で判断**: 「IT業界じゃない人が聞いて、何のことかピンとくるか？」
   - ピンと来ない → 補足必須
   - 例: 「API」「トークン」「コンテキストウィンドウ」「エージェント」「ハンズオン」なども要注意

**適用済み**: ch03-mcp-tools.html（npm / pip / パッケージ / settings.json / 環境構築 / Node.js / Python / CLI / コマンド・引数 / サーバー）
